...
export const prepareVideoPlayer = async (event, options) => {
  ...
  // Force video quality
  if (options.force_video_quality)
    forceVideoQuality(playerWrapper, options.video_quality);
...
const injectAudioControls = (playerWrapper, playbackRate) => {
  ...
  const playerTitle = playerWrapper.querySelector(
...
const injectVideoControls = (playerWrapper, canBeDownloaded, playbackRate) => {
  ...
  //   controls.lastElementChild.previousElementSibling.insertAdjacentHTML(
  //     "beforeBegin",
  //     templates.pipButton()
  //   );
  //   playerWrapper
  //     .querySelector("div.MB_pip")
  //     .addEventListener("click", preparePip);
  // }
  ...
  if (canBeDownloaded) {
    controls.lastElementChild.previousElementSibling.insertAdjacentHTML(
      "beforeBegin",
      templates.videoDownloadButton()
    );
    playerWrapper
      .querySelector("div.MB_download")
      .addEventListener("click", () => prepareVideoDownload(playerWrapper));
  }

  // Add and initialize speed controller
  controls.lastElementChild.previousElementSibling.insertAdjacentHTML(
    "beforebegin",
    templates.videoSpeedController(playbackRate)
  );
  ...
  controls.lastElementChild.previousElementSibling?.classList
    .remove("controls-element-indent-right")
...
const injectVideoDownloadModal = (links) => {
  ...
  const closeButton = document.querySelector(
    "button[class*=MediaSwiper_closeButton_]"
  );
  if (closeButton) closeButton.click();

  // FIXME: вот тут ошибка
  // VideoBlock-scss--module_root_YTcCW
  document
    .querySelector("div[class^=VideoBlock-]")
    .insertAdjacentHTML("beforeEnd", templates.VideoDownloadButton(links));

  const videoDownloadModal = document.querySelector("div#MB_video_download");
  const videoDownloadClose = document.querySelector(
    "span#MB_video_download_close"
  );
  ...
  const videoDownloadLinks = videoDownloadModal.querySelectorAll(
    "button.MB_video_download_link"
  );
...
const playContentEvent = (content, playerWrapper) => {
  content.addEventListener(
    "timeupdate",
    () => contentIsLoaded(content, playerWrapper),
    { once: true }
  );
};
...
const playbackSpeedController = (playerWrapper, player, playbackRate) => {
  ...
  const playbackRateElement = playerWrapper.querySelector(
    ".MB_current_playback_rate"
  );
  ...
  decreaseButton.addEventListener("click", (event) => {
    ...
    player.playbackRate =
      player.playbackRate - 0.25 > 0.25 ? player.playbackRate - 0.25 : 0.25;
  ...
  increaseButton.addEventListener("click", (event) => {
    ...
    player.playbackRate =
      player.playbackRate + 0.25 < 4.0 ? player.playbackRate + 0.25 : 4.0;
    changePlaybackRate(player.playbackRate);
  });
...
const changePlaybackRate = async (playbackRate) => {
  ...
  const audioDisplays = document.querySelectorAll(
    ".MB_current_playback_rate span"
  );
  ...
  for (const player of videoPlayers) {
    ...
    const display = player.shadowRoot.querySelector(
      ".MB_current_playback_rate span"
    );
    if (display) displaysList.push(display);
  }
